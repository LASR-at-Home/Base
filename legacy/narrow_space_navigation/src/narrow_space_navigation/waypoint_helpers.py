...
def draw_edges_on_window(window, edges):
    ...
    plt.savefig(
        "/home/nicole/robocup/rexy/waypoints/waypoints" + str(TEST) + "-4.jpg",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
def draw_contours(contours):
    ...
    plt.savefig(
        "/home/nicole/robocup/rexy/waypoints/waypoints-contours" + str(TEST) + "-5.jpg",
        dpi=300,
        bbox_inches="tight",
    )
...
def get_footprint(msg):
    footprint = rospy.wait_for_message(
        "/move_base/global_costmap/footprint", PolygonStamped
    )
...
def show_robot_footprint(window, footprint):
    ...
    plt.savefig(
        "/home/nicole/robocup/rexy/footprint" + str(TEST) + ".jpg",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
def area_with_shoelace(msg, elevator_points):
    ...
    for i in range(num_footprint_vertices):
        ...
        area += (
            footprint[i].flatten()[0] * footprint[j].flatten()[1]
            - footprint[j].flatten()[0] * footprint[i].flatten()[1]
        )
...
def line_intersect_poly(centers, dilation):
    ...
    for line in lines:
        ...
        if all(
            ...
            for x3, y3 in centers
            for x4, y4 in centers
            if (
                (x3, y3) != (line[0][0], line[0][1])
                and (x4, y4) != (line[1][0], line[1][0])
            )
        ):
    ...
    for combination in combinations:
        ...
        for side in sides:
            ...
            if is_outside_window(
                width, height, intersection_point[0], intersection_point[1]
            ):
                print("outside window", intersection_point)
                intersection_point = handle_outside_window(
                    width, height, intersection_point[0], intersection_point[1]
                )
