...
from tf_module.srv import (
    BaseTransformRequest,
    ApplyTransformRequest,
    LatestTransformRequest,
    BaseTransform,
    LatestTransform,
    ApplyTransform,
)
...
def point_cloud_callback(pcl_data):
    ...
    for point in point_cloud2.read_points(
        pcl_data, field_names=("x", "y", "z"), skip_nans=True
    ):
        dist = sqrt(
            (point[0] - door_location.x) ** 2 + (point[1] - door_location.y) ** 2
        )
...
def pub_markers(points, is_point=True):
    if is_point:
        for i in range(len(points)):
            door_pose_laser.publish(
                create_point_marker(points[i].x, points[i].y, points[i].z, i)
            )
    else:
        for i, pose in enumerate(points):
            pcl_to_point.publish(
                create_point_marker(
                    pose.position.x, pose.position.y, pose.position.z, i
                )
            )
...
def is_door_detected(laser_data):
    ...
    if door_detected != prev_door_detected:
        rospy.loginfo(
            "Door state changed: {} -> {}".format(prev_door_detected, door_detected)
        )
...
def transform_pointcloud_to_map(pcl, source_frame="xtion_rgb_optical_frame"):
    pss = []
    for point in point_cloud2.read_points(
        pcl, field_names=("x", "y", "z"), skip_nans=True
    ):
