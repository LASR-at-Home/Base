...
def save_navigation_points(number):
    ...
    if table >= 0:
        rospy.set_param(
            "/tables/table" + str(table) + "/location/position",
            {"x": position.x, "y": position.y, "z": 0.0},
        )
        rospy.set_param(
            "/tables/table" + str(table) + "/location/orientation",
            {
                "x": orientation.x,
                "y": orientation.y,
                "z": orientation.z,
        ...
        rospy.set_param(
            "/counter/location/position", {"x": position.x, "y": position.y, "z": 0.0}
        )
        rospy.set_param(
            "counter/location/orientation",
            {
                "x": orientation.x,
                "y": orientation.y,
                "z": orientation.z,
        ...
        rospy.set_param(
            "/wait/location/position", {"x": position.x, "y": position.y, "z": 0.0}
        )
        rospy.set_param(
            "wait/location/orientation",
            {
                "x": orientation.x,
                "y": orientation.y,
                "z": orientation.z,
...
def save_pre_navigation_points(number):
    ...
    if table >= 0:
        rospy.set_param(
            "/tables/table" + str(table) + "/pre_location/position",
            {"x": position.x, "y": position.y, "z": 0.0},
        )
        rospy.set_param(
            "/tables/table" + str(table) + "/pre_location/orientation",
            {
                "x": orientation.x,
                "y": orientation.y,
                "z": orientation.z,
        ...
        rospy.loginfo(
            "Pre-navigation point for table %d saved to parameter server", table
        )
...
if __name__ == "__main__":
    ...
    rospy.Service(
        "save_pre_navigation_points", SaveNavigationPoint, save_pre_navigation_points
    )
