...
from base.src.Base.common.navigation.choosing_wait_position.src.choosing_wait_position.final_lift_key_point.pytorch_utils.utils import (
    collate_fn,
)
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            ...
            keypoints_transformed_unflattened = np.reshape(
                np.array(transformed["keypoints"]), (-1, 2, 2)
            ).tolist()

            # Converting transformed keypoints from [x, y]-format to [x,y,visibility]-format by appending original visibilities to transformed coordinates of keypoints
            keypoints = []
            for o_idx, obj in enumerate(
                keypoints_transformed_unflattened
            ):  # Iterating over objects
                obj_keypoints = []
                for k_idx, kp in enumerate(
                    obj
        ...
        target["labels"] = torch.as_tensor(
            [1 for _ in bboxes], dtype=torch.int64
        ...
        target_original["labels"] = torch.as_tensor(
            [1 for _ in bboxes_original], dtype=torch.int64
        )  # all objects are glue tubes
...
KEYPOINTS_FOLDER_VALIDATE = "data/valid"
dataset_valid = ClassDataset(KEYPOINTS_FOLDER_VALIDATE, transform=None, demo=False)
data_loader_valid = DataLoader(
    dataset_valid, batch_size=3, shuffle=True, collate_fn=collate_fn
)


model = get_model(1, weights_path="models/keypointsrcnn_weights.pth")
...
high_scores_idxs = np.where(scores > 1.3)[
    0
].tolist()  # Indexes of boxes with scores > 0.7
post_nms_idxs = (
    torchvision.ops.nms(
        output[0]["boxes"][high_scores_idxs], output[0]["scores"][high_scores_idxs], 0.3
    )
    .cpu()
    .numpy()
...
keypoints = []
for kps in (
    output[0]["keypoints"][high_scores_idxs][post_nms_idxs].detach().cpu().numpy()
):
