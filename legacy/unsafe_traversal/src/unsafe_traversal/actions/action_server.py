...
import rospy
import actionlib
from unsafe_traversal.srv import (
    ChangeTraversalParameters,
    DeterminePathViability,
    LaserDist,
)
from move_base_msgs.msg import MoveBaseAction, MoveBaseGoal
from unsafe_traversal.msg import (
    MoveToGoalAction,
    AlignToGoalAction,
    MoveToGoalGoal,
    AlignToGoalGoal,
...
class TraversalActionServer(MoveBaseClient):
    ...
    def __init__(self):
        ...
        self._align_server = actionlib.SimpleActionServer(
            "/unsafe_traversal/align_to_goal",
            AlignToGoalAction,
            execute_cb=self.align_action_cb,
            auto_start=False,
        )
        self._move_server = actionlib.SimpleActionServer(
            "/unsafe_traversal/move_to_goal",
            MoveToGoalAction,
            execute_cb=self.move_action_cb,
            auto_start=False,
        )
        ...
        self.viable_plan_srv = rospy.ServiceProxy(
            "/unsafe_traversal/check_if_plan_is_viable", DeterminePathViability
        )
        self.laser_dist_srv = rospy.ServiceProxy(
    ...
    def move_action_cb(self, msg):
        ...
        laser_dist = self.laser_dist_srv(60.0).dist
