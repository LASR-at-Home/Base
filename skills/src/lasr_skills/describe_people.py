...
class DescribePeople(yasmin.StateMachine):
    def __init__(self):
        ...
        loop_state = yasmin.CbState(outcomes=['succeeded', 'continue'], callback=self._get_attr)
...
class GetClipAttributes(yasmin_ros.ServiceState):
    ...
    def _handle_resp(self, blackboard, response):
        if blackboard['clip_index'] == 0:
            ...
            blackboard['clip_detection_dict'].update({'short_sleeve_t_shirt': t_shirt_bool})
