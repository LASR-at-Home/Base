...
if PUBLIC_CONTAINER or SIMULATION:
    ...
    class Say(smach.State):
        ...
        def __init__(
            ...
            if PUBLIC_CONTAINER:
                rclpy.logging.get_logger('Say').warning("You are using the public container, the Say skill will not work")
            
            elif SIMULATION:
                rclpy.logging.get_logger('Say').warning("You are using the simulation, the Say skill will not work")
        ...
        def execute(self, userdata):
            if self.text is not None:
                ...
                rclpy.logging.get_logger('Say').info(self.format_str.format(userdata.placeholders))
            else:
...
def main(args=None):
    ...
    with sm:
        smach.StateMachine.add('Say', Say(),
            transitions={'preempted': 'preempted', 'succeeded':'succeeded', 'aborted':'aborted'},
        )
