...
class WaitForPerson(smach.StateMachine):
    ...
    def __init__(self):
        ...
        smach.StateMachine.__init__(self, outcomes=['succeeded', 'failed'], input_keys=['depth_topic'], output_keys=['detections'])

        with self:
            smach.StateMachine.add('DETECT_PEOPLE', DetectPeople(), transitions={'succeeded' : 'CHECK_FOR_PERSON', 'failed' : 'failed'})
            smach.StateMachine.add('CHECK_FOR_PERSON', self.CheckForPerson(), transitions={'done' : 'succeeded', 'not_done' : 'DETECT_PEOPLE'})