...
class EvaluatePosture2D(yasmin.State):
    ...
    def execute(self, blackboard):
        ...
        if not all(k in kp for k in ['right_shoulder', 'left_shoulder', 'right_wrist', 'left_wrist', 'right_hip', 'right_knee']):
            return "failed"

        shoulder_width = abs(kp['right_shoulder'].x - kp['left_shoulder'].x) + 1e-6
        
        candidates = {"waving": False, "Pointing": False, "Raising Arm": False, "Sitting": False}
        pointing_dir = "None"

        # 1. Waving 판정 (self.is_waving으로 호출)
        if self.is_waving(kp['right_wrist'], kp['right_shoulder'], self.r_history, shoulder_width) or \
           self.is_waving(kp['left_wrist'], kp['left_shoulder'], self.l_history, shoulder_width):
            candidates["waving"] = True

        # 2. Pointing 판정
        if not candidates["waving"]:
            is_arm_horizontal = abs(kp['right_wrist'].y - kp['right_shoulder'].y) < (shoulder_width * 0.7)
            if is_arm_horizontal:
                if kp['right_wrist'].x > kp['right_shoulder'].x + (shoulder_width * 0.2):
                    candidates["Pointing"] = True
                    pointing_dir = "Left"
                elif kp['right_wrist'].x < kp['right_shoulder'].x - (shoulder_width * 0.2):
                    candidates["Pointing"] = True
                    pointing_dir = "Right"
        ...
        # 최종 판정
        if candidates["waving"]: detected_posture = "waving"
        elif candidates["Pointing"]: detected_posture = f"Pointing {pointing_dir}"
        elif candidates["Raising Arm"]: detected_posture = f"Raising Arm {arm_dir}" # 방향 포함!
        elif candidates["Sitting"]: detected_posture = "Sitting"
        else: detected_posture = "Standing"
