...
class SamDecoder:
    def __init__(self, model_path: str, device: str = "cpu", target_size: int = 1024, mask_threshold: float = 0.0, **kwargs):
        opt = ort.SessionOptions()
        provider = ["CUDAExecutionProvider"] if device == "cuda" else ["CPUExecutionProvider"]
    ...
    def run(self, img_embeddings, origin_image_size, point_coords=None, point_labels=None, boxes=None, return_logits=False):
        ...
        low_res_masks, iou_predictions = self.session.run(None, {
        ...
        import sys
        print(f"[SAM] origin_image_size={origin_image_size}, input_size={input_size}, low_res_masks shape={low_res_masks.shape}, low_res min={low_res_masks.min():.3f} max={low_res_masks.max():.3f} mean={low_res_masks.mean():.3f}, point_coords={point_coords.tolist() if point_coords is not None else None}", flush=True)
        sys.stdout.flush()
        masks = _mask_postprocessing(low_res_masks, origin_image_size, img_size=self.target_size)
        if not return_logits:
            masks = masks > self.mask_threshold
        return masks, iou_predictions, low_res_masks


def _mask_postprocessing(masks: np.ndarray, orig_im_size, img_size: int = 1024) -> np.ndarray:
...
def _resize_longest_image_size(input_image_size: torch.Tensor, longest_side: int) -> torch.Tensor:
...
class VitSam:
    ...
    def __call__(self, image: np.ndarray, bboxes: List) -> Tuple[np.ndarray, np.ndarray]:
        ...
        masks, _, _ = self.decoder.run(img_embeddings=img_embeddings, origin_image_size=origin_image_size, boxes=boxes)
        return masks, boxes
