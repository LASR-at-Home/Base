...
class TestDetect(Node):
    def __init__(
        ...
        queries,
        image_path=None,
        output_path="/tmp/open_vocab_result.jpg",
        segment=False,
    ):
        ...
        if image_path is None:
            ...
            self.create_subscription(
                ...
                lambda msg: setattr(self, "_latest_image", msg),
                camera_qos,
            )
    ...
    def run(self):
        ...
        if self.segment and result.masks:
            ...
            for i, mask_msg in enumerate(result.masks):
                ...
                mask_path = (
                    self.output_path.replace(".jpg", f"_mask_{i}.jpg")
                    .replace(".jpeg", f"_mask_{i}.jpeg")
                    .replace(".png", f"_mask_{i}.png")
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test open_vocab detect/segment service"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to image file (omit to use Tiago camera)",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="/tmp/open_vocab_result.jpg",
        help="Output image path",
    )
    parser.add_argument(
        "--segment", action="store_true", help="Use detect_and_segment service"
    )
