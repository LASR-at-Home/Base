...
class ViablePlanCheckerService:
    ...
    def __init__(self):
        ...
        self.service = rospy.Service(
            "/unsafe_traversal/check_if_plan_is_viable",
    ...
    def check_plan(self, request):
        ...
        # create the plan
        plan = self.proxy(
            request.start_pose, request.end_pose, MOVE_BASE_TOLERANCE
        ).plan
        ...
        max_dist = self.euclidian_distance(
            request.start_pose.pose.position, request.end_pose.pose.position
        )
