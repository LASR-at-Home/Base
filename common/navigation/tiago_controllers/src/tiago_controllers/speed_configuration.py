...
class SpeedConfiguration:
    def __init__(self, speed_mode):
        self.client = dynamic_reconfigure.client.Client(
            "/move_base/PalLocalPlanner", timeout=30, config_callback=self.callback()
        )
    ...
    def is_within_tol(self, value, default=0.0):
        return (
            abs(value) <= abs(default * MAX_CHANGE)
            if default != 0.0
            else abs(value) <= abs(default + MAX_CHANGE)
        )

    def change_robot_speed(self, param, value, tol=0.1):
        try:
            rospy.logwarn(
                f"Config before change of {param} set to {self.client.get_configuration()[param]}"
            )
            print("   ")
            min_param = [
                x
                for x in self.client.get_parameter_descriptions()
                if x["name"].lower() == param.lower()
            ][0].get("min")
            max_param = [
                x
                for x in self.client.get_parameter_descriptions()
            ...
            if not max_param > value > min_param or not self.is_within_tol(
                value, default
            ):  # or type(value) != type_param
