...
import rospy
from tiago_controllers.controllers.base_controller import (
    BaseController,
    CmdVelController,
)
...
class LookAt:
    ...
    def aim_head(self):
        ...
        if tmp_hum_x and tmp_hum_y:
            ...
            self.__move_robot_head(
                hum_centroid[0], hum_centroid[1], tmp_xywh, robot_pos
            )
    ...
    def __move_robot_head(self, x, y, _xywh, robot_pos):
        try:
            ...
            print(
                f"the human pos is h = {xywh[3]}, and the w = {xywh[2]}, and all of xywh = {xywh}"
            )
            i = 0
            moving_incr_x = 0.0
            moving_incr_y = 0.0
            ...
            while (
                abs(xywh[0] + (xywh[2] // 2) - rob_w) > 50
                and abs(xywh[1] + xywh[3] - rob_h) > 25
            ):
                print(
                    f"in the while and {abs(xywh[0] + (xywh[2]//2) - rob_w)} and the y axis {abs(xywh[1] + xywh[2] - rob_h)} is bigger than 25"
                )
    ...
    def check_moving_side(
        self, hum_x, hum_y, rob_x, rob_y, _moving_incr_x, _moving_incr_y
    ):
        try:
