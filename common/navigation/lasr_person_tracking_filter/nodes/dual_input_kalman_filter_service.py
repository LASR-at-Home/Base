...
from lasr_person_tracking_filter.srv import DualSensorPersonTrackingFilter, DualSensorPersonTrackingFilterResponse
...
class PersonTrackingDualInputKalmanFilterService:
    ...
    def __init__(self):
        ...
        self.sensor_buffer_window = rospy.Duration.from_sec(0.5)  # 500ms window for temporal fusion
        self.max_temporal_gap = rospy.Duration.from_sec(2.0)  # 2s max gap before considering sensor dead
    ...
    def handle_filter_request(self, req):
        ...
        try:
            if req.command == "initialize":
                ...
                response.success = success
                response.message = "Dual sensor async update" if success else "Dual update failed"
        ...
        try:
            ...
            if sensor_id == 'a':
                self.last_sensor_a.update({
                    'x': x, 'y': y, 'time': timestamp, 'quality': quality,
    ...
    def _process_asynchronous_update(self, current_time):
        ...
        try:
            ...
            fusion_strategy = self._analyze_temporal_fusion_strategy(viable_sensors, current_time)
    ...
    def _get_viable_sensors(self, current_time):
        ...
        if (self.last_sensor_a['time'] is not None and
                self.last_sensor_a['x'] is not None and
                (current_time - self.last_sensor_a['time']) < self.max_temporal_gap):
            viable.append('a')

        # Check sensor B viability
        if (self.last_sensor_b['time'] is not None and
                self.last_sensor_b['x'] is not None and
                (current_time - self.last_sensor_b['time']) < self.max_temporal_gap):
            viable.append('b')
    ...
    def _analyze_temporal_fusion_strategy(self, viable_sensors, current_time):
        ...
        if len(viable_sensors) == 0:
            ...
            projected_a = self._project_measurement_to_time(self.last_sensor_a, current_time)
            projected_b = self._project_measurement_to_time(self.last_sensor_b, current_time)
            ...
            if projected_a and projected_b:
                ...
                if distance < self.agreement_thresholds['good_agreement'] and temporal_sync:
                    fusion_type = 'optimal'
                elif distance < self.agreement_thresholds['poor_agreement']:
                    fusion_type = 'cautious'
                else:
                    # Large disagreement - choose better sensor
                    score_a = self.sensor_reliability['a'] * weight_a * self.last_sensor_a['quality']
                    score_b = self.sensor_reliability['b'] * weight_b * self.last_sensor_b['quality']

                    better_sensor = 'a' if score_a > score_b else 'b'
                    return {
                        'type': 'conflict_single',
                        'sensor_id': better_sensor,
                        'data': self.last_sensor_a if better_sensor == 'a' else self.last_sensor_b,
    ...
    def _apply_temporal_fusion(self, strategy, current_time):
        ...
        try:
            if strategy['type'] == 'single' or strategy['type'] == 'conflict_single':
                ...
                age_factor = 1.0 + strategy.get('age', 0) * 0.1  # 10% penalty per second

                noise = base_noise * temporal_factor * age_factor / max(sensor_data['quality'], 0.1)
                ...
                noise_a = base_noise / (strategy['weight_a'] * max(data_a['quality'], 0.1))
                noise_b = base_noise / (strategy['weight_b'] * max(data_b['quality'], 0.1))
            ...
            if 'sensor_id' in strategy:
                self.sensor_reliability[strategy['sensor_id']] = min(1.0,
                                                                     self.sensor_reliability[
                                                                         strategy['sensor_id']] + 0.01)
            else:
                self.sensor_reliability['a'] = min(1.0, self.sensor_reliability['a'] + 0.01)
                self.sensor_reliability['b'] = min(1.0, self.sensor_reliability['b'] + 0.01)
    ...
    def _initialize_with_any_available_sensor(self, timestamp):
        ...
        # Try sensor A first
        if (self.last_sensor_a['x'] is not None and
                self.last_sensor_a['time'] is not None):
            return self._initialize(self.last_sensor_a['x'], self.last_sensor_a['y'], timestamp)

        # Try sensor B
        elif (self.last_sensor_b['x'] is not None and
              self.last_sensor_b['time'] is not None):
            return self._initialize(self.last_sensor_b['x'], self.last_sensor_b['y'], timestamp)
    ...
    def _initialize(self, x, y, timestamp):
        ...
        try:
            ...
            self.consecutive_conflicts = 0
            self.prediction_mode = False
            rospy.loginfo(f"Dual Kalman filter initialized at position ({x:.2f}, {y:.2f})")
            return True
        except Exception as e:
