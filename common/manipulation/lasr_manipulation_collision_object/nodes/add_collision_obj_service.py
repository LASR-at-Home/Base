...
from lasr_manipulation_collision_object.srv import (
    GenerateCollisionObject,
    GenerateCollisionObjectRequest,
    GenerateCollisionObjectResponse,
)
    ...
    points = np.array(
        [
            ...
            for pt in pc2.read_points(
                pc2_msg, field_names=("x", "y", "z"), skip_nans=True
            )
        ]
    )
...
def handle_completion_to_collision(
    req: GenerateCollisionObjectRequest,
) -> GenerateCollisionObjectResponse:
    rospy.loginfo(
        f"[collision_service] Received point cloud for object '{req.object_id}'"
    )
...
def main():
    ...
    rospy.Service(
        "completion_to_collision_object",
        GenerateCollisionObject,
        handle_completion_to_collision,
    )
    rospy.loginfo(
        "[collision_service] Service 'completion_to_collision_object' is ready."
    )
    rospy.spin()
