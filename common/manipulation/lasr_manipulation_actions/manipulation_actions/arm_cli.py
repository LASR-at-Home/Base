...
class ArmCLI(Node):
    def __init__(self):
        ...
        self._debug_marker_pub = self.create_publisher(MarkerArray, '/arm_cli_debug', 10)
        ...
        self.create_subscription(Detection3DArray, '/object_centroids', self._centroids_cb, 10)
    ...
    def get_ee(self):
        try:
            tf = self._tf_buffer.lookup_transform(
                'base_footprint', 'gripper_grasping_frame',
                ROS2Time(), timeout=rclpy.duration.Duration(seconds=1.0)
            )
    ...
    def move_joints_blocking(self, joints, timeout=15.0):
        self._moveit.move_to_configuration(joint_positions=joints, joint_names=ARM_JOINTS)
    ...
    def rotate_base_toward(self, target_map_x: float, target_map_y: float,
                           tolerance_rad: float = 0.03, max_omega: float = 0.08,
                           timeout: float = 25.0):
        """
        ...
        while time.time() < end:
            try:
                tf = self._tf_buffer.lookup_transform(
                    'map', 'base_footprint',
                    ROS2Time(), timeout=rclpy.duration.Duration(seconds=0.5)
                )
            except Exception:
                self._cmd_vel_pub.publish(Twist())
                return False
            ...
            if time.time() - last_print > 0.5:
                print(f'    yaw={yaw:+.3f} desired={desired:+.3f} err={err:+.3f} ω={omega:+.2f}')
    ...
    def drive_base_until_target(self, target_map_x: float, target_map_y: float,
                                stop_offset: float = 0.12,
                                linear_x: float = 0.08, tolerance: float = 0.04,
                                timeout: float = 20.0,
                ...
                if ee_bf is not None and t_bf is not None:
                    ...
                    if abs(dy) > align_tolerance:
                        ...
                        if abs(new_arm1 - joints[1]) > 1e-3:
                            ...
                            print(f'    ⟲ realign: dy={dy:+.3f} → arm_1={new_arm1:+.3f}')
                last_align = time.time()

            if time.time() - last_print > 0.5:
                print(f'    EE(map): ({ee_map.x:.3f},{ee_map.y:.3f}) → goal_map({goal_x:.3f},{goal_y:.3f})  dist={dist:.3f}')
                last_print = time.time()
            rclpy.spin_once(self, timeout_sec=0.02)
            time.sleep(0.05)
        # Always stop
    ...
    def align_arm1_to_target(self, tolerance: float = 0.01, max_iters: int = 5):
        ...
        for it in range(max_iters):
            ...
            print(f'  [iter {it+1}] dy={dy:+.3f}  Δarm_1={actual_delta:+.3f}  '
    ...
    def print_objects(self):
        ...
        if not self._centroids:
            ...
            print(f'  Run: ros2 topic pub --once /detect std_msgs/msg/String "data: \'bottle\'"')
        else:
            print(f'  {"#":<3} {"Name":<20} {"x":>6} {"y":>6} {"z":>6}')
            print(f'  {SEP}')
            for i, det in enumerate(self._centroids):
                marker = ' <-- TARGET' if self._target and det.name == self._target.name else ''
                print(f'  {i+1:<3} {det.name:<20} {det.point.x:>6.3f} {det.point.y:>6.3f} {det.point.z:>6.3f}{marker}')
        print(f'{SEP2}\n')
    ...
    def _publish_debug_markers(self, ee_map):
        ...
        if self._target is not None:
            ...
            if ee_map is not None:
                ...
                line.scale.x = 0.015
                line.color.r, line.color.g, line.color.b, line.color.a = 0.0, 1.0, 0.0, 0.9
                p1 = Point(x=ee_map.x, y=ee_map.y, z=ee_map.z)
                p2 = Point(x=self._target.point.x, y=self._target.point.y, z=self._target.point.z)
                line.points = [p1, p2]
    ...
    def print_status(self):
        ...
        print(f'    arm_1  : {joints[1]:>+7.3f} rad   arm_2: {joints[2]:>+7.3f}   arm_3: {joints[3]:>+7.3f}')
        print(f'    arm_4  : {joints[4]:>+7.3f} rad   arm_5: {joints[5]:>+7.3f}   arm_6: {joints[6]:>+7.3f}   arm_7: {joints[7]:>+7.3f}')
        ...
        if ee_map:
            print(f'    map    :  x={ee_map.x:>+.3f}  y={ee_map.y:>+.3f}  z={ee_map.z:>+.3f}')

        if self._target and ee:
            print(f'  {SEP}')
            print(f'  TARGET: {self._target.name}')
            print(f'    map    :  x={self._target.point.x:>+.3f}  y={self._target.point.y:>+.3f}  z={self._target.point.z:>+.3f}')
            t = self.get_target_in_base()
            if t is None:
                print(f'    base_fp:  [TF unavailable]')
            else:
                ...
                print(f'    dx={dx:>+.3f}  dy={dy:>+.3f}  dz={dz:>+.3f}   |xy|={dist_xy:.3f}m  |3d|={dist_3d:.3f}m')
                x_ok = '✓' if abs(dx) < 0.05 else '✗'
                y_ok = '✓' if abs(dy) < 0.05 else '✗'
                z_ok = '✓' if abs(dz) < 0.05 else '✗'
                print(f'    X {x_ok}     Y {y_ok}     Z {z_ok}    (✓ = within 5cm)')
                if abs(dy) >= 0.05:
                    print(f'    hint:    press {"l" if dy > 0 else "r"} to swing arm_1 toward target Y')
                if abs(dz) >= 0.05:
                    print(f'    hint:    press {"u" if dz > 0 else "d"} to move torso toward target Z')
            if ee_map:
                ...
                print(f'    dx={ddx:>+.3f}  dy={ddy:>+.3f}  dz={ddz:>+.3f}   |xy|={d_xy:.3f}m')
        elif self._target:
