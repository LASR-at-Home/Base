...
class GraspPlanner:
    ...
    def _get_ee_pose(self):
        ...
        try:
            tf = self._tf_buffer.lookup_transform(
                'base_footprint', 'arm_tool_link',
                ROS2Time(seconds=0), timeout=rclpy.duration.Duration(seconds=1.0)
            )
    ...
    def _align_y(self, target_y: float):
        ...
        for i in range(max_iter):
            ...
            self.node.get_logger().info(f'align_y iter {i}: ee_y={ee.y:.3f} target_y={target_y:.3f} y_delta={y_delta:.3f}')
