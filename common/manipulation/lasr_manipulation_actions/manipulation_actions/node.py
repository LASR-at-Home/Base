...
class ManipulationNode(Node):
    def __init__(self):
        ...
        self.create_subscription(
            Detection3DArray, "/object_centroids", self._on_centroids, 10
        )
        self.create_subscription(String, "/command", self._on_command, 10)

        self.get_logger().info(
            "ManipulationNode ready. Commands: home | init_grasp | grasp <object>"
        )
        ...
        try:
            if cmd == "home":
                ...
                self.get_logger().warn(
                    f'Unknown command: "{cmd}". Use: home | init_grasp | grasp <object>'
                )
    ...
    def _do_grasp(self, object_name: str):
        ...
        if match is None:
            self.get_logger().warn(
                f'Object "{object_name}" not found in /object_centroids'
            )
            return

        self.get_logger().info(
            f'Grasping "{match.name}" at ({match.point.x:.2f}, {match.point.y:.2f}, {match.point.z:.2f})'
        )
