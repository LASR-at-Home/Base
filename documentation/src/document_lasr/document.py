...
def generate_readme(pkg_dir):
    ...
    for child in tree.getroot():
        if child.tag == "name":
            ...
            data["maintainers"].append(
                {"name": child.text, "email": child.attrib["email"]}
            )
        elif child.tag == "author":
            data["authors"].append({"name": child.text, "email": child.attrib["email"]})
        elif child.tag in [
            "buildtool_depend",
            "depend",
            "build_depend",
            ...
            data["dependencies"].append({"package": child.text, "type": child.tag})
    ...
    if exists(PATH):
        ...
        python_dependencies = [
            split_depend(depend)
            for depend in python_requirements
            if split_depend(depend) is not None
        ]

        all_python_dependencies = [
            split_depend(depend)
            for depend in all_requirements
            if split_depend(depend) is not None
        ]
        ...
        hidden_dependencies = [
            name
            for (name, _) in all_python_dependencies
            if not any(name == pkg for (pkg, _) in python_dependencies)
        ]
    ...
    if exists(LAUNCH_FILES):
        ...
        for file in files:
            ...
            for child in tree.getroot():
                if child.tag == "description":
                    ...
                    launch_data["usage"].append(
                        {"description": child.attrib["doc"], "arguments": child.text}
                    )
                elif child.tag == "arg":
            ...
            launch_data["usage"] = (
                f"{LAUNCH_USAGE}```" if len(launch_data["usage"]) > 0 else ""
            )
            ...
            launch_data["arguments"] = (
                table if len(launch_data["arguments"]) > 0 else ""
            )
    ...
    def parse_rosmsg_definition(input):
        ...
        for line in input.split("\n"):
            ...
            # parse type / name
                ...
                var_type, var_name = [
                    seg for seg in definition.strip().split(" ") if len(seg) > 0
                ]
                MESSAGE[var_name] = {
                    "type": var_type,
