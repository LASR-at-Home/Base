    ...
    def __init__(self):
      ...
      #  self.userdata.area_polygon = [[1.94, 0.15], [2.98, 0.28], [3.08, -0.68], [2.06, -0.84]]
       # self.userdata.depth_topic = "/xtion/depth_registered/points"
        self.userdata.wait_location = get_pose_from_param_new('/wait_position/pose')
        self.userdata.person_location = get_pose_from_param_new('/talk_position/pose')
        self.userdata.seat_location = get_pose_from_param_new('/seating_area_position/pose')
        with self:
            #Phase 1: 
           # smach.StateMachine.add('START', Start(self.default), transitions={'succeeded' : 'GO_TO_WAIT_FOR_PERSON'})
            smach.StateMachine.add('GO_TO_WAIT_FOR_PERSON',GoToLocation(),transitions={'failed' : 'GO_TO_LOCATION','succeeded': 'WAIT_FOR_PERSON'},remapping={"location":"wait_location"})
            smach.StateMachine.add('WAIT_FOR_PERSON', WaitForPersonInArea([[1.94, 0.15], [2.98, 0.28], [3.08, -0.68], [2.06, -0.84]]) ,transitions={'failed' : 'failed','succeeded':'FACE_PERSON'})
            smach.StateMachine.add('GO_TO_PERSON', GoToLocation(),transitions={'succeeded':'DETECT_FACES'},remapping={'location':'person_location'})
