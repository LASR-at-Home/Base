...
rospy.logwarn(
    "DEBUG: {DEBUG}, DEBUG_WITH_IMAGES: {DEBUG_WITH_IMAGES}, PLOT_SHOW: {PLOT_SHOW}, PLOT_SAVE: {PLOT_SAVE}".format(
        **locals()
    )
)
...
if DEBUG_WITH_IMAGES:
    if not os.path.exists(
        os.path.join(rospkg.RosPack().get_path("lift"), "debug_lift")
    ):
        os.mkdir(os.path.join(rospkg.RosPack().get_path("lift"), "debug_lift"))
    DEBUG_PATH = os.path.join(
        rospkg.RosPack().get_path("lift"),
        "debug_lift",
    )
