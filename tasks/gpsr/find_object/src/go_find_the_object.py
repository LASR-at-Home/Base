...
class Go_find_the_object(smach.StateMachine):
    class check_objects(smach.State):
        def __init__(self):
            smach.State.__init__(self, outcomes=['succeeded', 'failed'], input_keys=['detections_3d'], output_keys=['detection_result'])
    ...
    class detection_result(smach.State):
        def __init__(self): 
            smach.State.__init__(self, outcomes=['object_found', 'object_not_found', 'failed'], input_keys=['cumulated_result'], output_keys=['result','cumulated_result'])
    ...
    def __init__(
        ...
        with self:
            ...
            for i, location in enumerate(locations):
                ...
                smach.StateMachine.add(f'LOCATION_{i}', GoToLocation(pose), transitions={'succeeded': f"LOOK_AROUND_{state}", 'aborted': 'failed', 'preempted': 'failed'})
                
                # Inner loop iterates over motions
                for index, direction in enumerate(motions):
                    print(direction)
                    # Add a state to perform the current motion
                    smach.StateMachine.add(f'LOOK_AROUND_{state}', PlayMotion(motion_name=direction), transitions={'succeeded': f"DETECT_OBJECTS_3D_{state}", 'aborted': f"DETECT_OBJECTS_3D_{state}", 'preempted': 'failed'})
                    
                    # Add a state to detect 3D objects after performing the motion
                    smach.StateMachine.add(f"DETECT_OBJECTS_3D_{state}", Detect3D(depth_topic=depth_topic, model=model, filter=filter, confidence=confidence, nms=nms), transitions={'succeeded': f'RESULT_{state}', 'failed': 'failed'})

                    # Add a state to check objects
                    smach.StateMachine.add(f'RESULT_{state}', self.check_objects(), transitions={'succeeded': f'SAVE_RESULT_{state}', 'failed' : 'failed'})
                    smach.StateMachine.add(f'SAVE_RESULT_{state}', self.cumulate_result(), transitions={'succeeded': f'LOOK_AROUND_{state+1}' if index != (len(motions) - 1) else f'CHECK_RESULT_{i}', 'failed' : 'failed'})
                    state += 1

                smach.StateMachine.add(f'CHECK_RESULT_{i}', self.detection_result(), transitions={'object_found': f'SAY_OBJECT_FOUND_{i}', 'object_not_found': f'SAY_OBJECT_NOT_FOUND_{i}', 'failed': 'failed'})
                    # Add states to handle object found or not found results
                smach.StateMachine.add(f'SAY_OBJECT_FOUND_{i}', say_object_found, transitions={'succeeded': f'LOCATION_{i+1}' if i != len(locations) - 1 else 'succeeded', 'aborted': 'failed', 'preempted': 'failed'})
                smach.StateMachine.add(f'SAY_OBJECT_NOT_FOUND_{i}', say_object_not_found, transitions={'succeeded': f'LOCATION_{i+1}' if i != len(locations) - 1 else 'succeeded', 'aborted': 'failed', 'preempted': 'failed'})
...
if __name__ == "__main__":
    ...
    sm = Go_find_the_object(Polygon(), filter=["cup"], locations=location_list, motions=motion)
