...
class ObjectComparison(smach.StateMachine):
    ...
    class CountCategory(smach.State):
        def __init__(self, object_weight: Union[List[dict], None] = None):
            smach.State.__init__(self, outcomes=['succeeded', 'failed'], input_keys=['object_dict'], output_keys=['category_dict', 'detections_categories'])
    ...
    class ObjectWeight(smach.State):
        ...
        def execute(self, userdata):
            weights_dict = self.get_weight(userdata.detections_types, self.object_weight)
    ...
    def __init__(
        ...
        with self:
            ...
            smach.StateMachine.add('COUNTOBJECTS', self.CountObjectTypes(area_polygon), transitions={'succeeded' : 'COUNTCATEGORY', 'failed' : 'failed'})
            smach.StateMachine.add('COUNTCATEGORY', self.CountCategory(object_weight=object), transitions={'succeeded' : 'GETWEIGHT', 'failed' : 'failed'})
            smach.StateMachine.add('GETWEIGHT', self.ObjectWeight(object_weight=object), transitions={'succeeded' : 'GETSIZE', 'failed' : 'failed'})
            smach.StateMachine.add('GETSIZE', self.ObjectSize(), transitions={'succeeded' : 'succeeded', 'failed' : 'failed'})
