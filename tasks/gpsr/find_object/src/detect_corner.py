...
class DetectTurn(smach.State):
    ...
    def odom_callback(self, msg):
        ...
        if self.prev_orientation is not None:
            ...
            if abs(yaw_diff) > self.turn_threshold:
                rospy.loginfo("Turn detected: {:.2f} degrees".format(math.degrees(yaw_diff)))
    ...
    def imu_callback(self, msg):
        ...
        if abs(angular_velocity_z) > self.turn_threshold:
            rospy.loginfo("Turn detected from IMU: {:.2f} degrees/sec".format(math.degrees(angular_velocity_z)))
            self.turn_detected = True
...
class TrackDistance(smach.State):
    ...
    def odom_callback(self, msg):
        if self.start_position is None:
            ...
            distance_travelled = self.calculate_distance(self.start_position, current_position)
