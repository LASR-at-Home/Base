...
class Guiding_people(smach.StateMachine):
    class Detect_people(smach.State):
        ...
        def __init__(self):
            smach.State.__init__(
                self,
                outcomes=["people_found", "people_not_found", "failed"],
    ...
    class detection_result(smach.State):
        def __init__(self):
            smach.State.__init__(
                self,
                outcomes=["check_found", "check_not_found", "failed"],
    ...
    def __init__(
        ...
        with self:
            # 1. Start & greeting
            start_pose = Pose(
                position=Point(**locations[0]["pose"]["position"]),
                orientation=Quaternion(**locations[0]["pose"]["orientation"]),
            )
            smach.StateMachine.add(
                "START_GUIDING",
                GoToLocation(start_pose),
                transitions={
                    "succeeded": "GREETING_GUEST_FIRST",
                    "aborted": "failed",
                    "preempted": "failed",
                },
            )
            smach.StateMachine.add(
                "GREETING_GUEST_FIRST",
                Say(text="I am waiting for a person."),
                transitions={
            ...
            smach.StateMachine.add(
                ...
                PlayMotion(motion_name=motions[0]),
                transitions={
                    "succeeded": "GREETING_GUEST",
                    "aborted": "GREETING_GUEST",
                    "preempted": "failed",
                },
            )
            smach.StateMachine.add(
                "GREETING_GUEST",
                Say(text="Please stand in front of me."),
                transitions={
                    "succeeded": "DETECT_OBJECTS_3D_START",
                    "aborted": "failed",
                    "preempted": "failed",
                },
            )
            ...
            smach.StateMachine.add(
                "DETECT_OBJECTS_3D_START",
                WaitForPersonInArea(waiting_area[0]),
                transitions={
                    "find_person": "GREETING_GUEST_YES",
                    "not_find_person": "GREETING_GUEST_NO",
                    "failed": "failed",
                },
            )
            smach.StateMachine.add(
                "GREETING_GUEST_YES",
                Say(text="Hello, nice to meet you, please follow me."),
                transitions={
                    "succeeded": "START_GUIDING_0",
                    "aborted": "failed",
                    "preempted": "failed",
                },
            )
            ...
            for i, location in enumerate(locations[1:-1]):
                ...
                turn_pose = Pose(
                    position=Point(**turning_point[i]["pose"]["position"]),
                    orientation=Quaternion(**turning_point[i]["pose"]["orientation"]),
                )
                smach.StateMachine.add(
                    f"TURNING_AROUND{i}",
                    GoToLocation(turn_pose),
                    transitions={
                        "succeeded": f"CHECK_AROUND_{i}_0",
                        "aborted": "failed",
                        "preempted": "failed",
                    },
                )
                ...
                for index, motion in enumerate(motions):
                    smach.StateMachine.add(
                        f"CHECK_AROUND_{i}_{state}",
                        PlayMotion(motion_name=motion),
                        transitions={
                            "succeeded": f"DETECT_OBJECTS_{i}_{state}",
                            "aborted": f"DETECT_OBJECTS_{i}_{state}",
                            "preempted": "failed",
                        },
                    )
                    smach.StateMachine.add(
                        f"DETECT_OBJECTS_{i}_{state}",
                        WaitForPersonInArea(waiting_area[i + 1]),
                        transitions={
                            "find_person": f"RESULT_{i}_{state}",
                            "not_find_person": f"RESULT_{i}_{state}",
                            "failed": "failed",
                        },
                    )
                    smach.StateMachine.add(
                        ...
                        self.Detect_people(),
                        transitions={
                            "people_found": f"SAVE_RESULT_{i}_{state}",
                            "people_not_found": f"SAVE_RESULT_{i}_{state}",
                            "failed": "failed",
                        },
                    )
                    smach.StateMachine.add(
                        f"SAVE_RESULT_{i}_{state}",
                        self.cumulate_result(),
                        transitions={
                            "succeeded": (
                                f"CHECK_AROUND_{i}_{state+1}"
                ...
                smach.StateMachine.add(
                    ...
                    transitions={
                        "check_found": f"GREETING_GUEST_YES_{i}",
                        "check_not_found": f"GREETING_GUEST_NO_{i}",
                        "failed": "failed",
                    },
                )
                smach.StateMachine.add(
                    f"GREETING_GUEST_YES_{i}",
                    Say(text="Let's move on to the destination."),
                    transitions={
                        "succeeded": (
                            f"START_GUIDING_{i+1}"
                            if index != (len(locations[1:-1]))
                            else "GUIDING_DESTINATION"
                        ),
                        "aborted": "failed",
                        "preempted": "failed",
                    },
                ...
                smach.StateMachine.add(
                    f"GREETING_GUEST_NO_{i}",
                    Say(text="Hey, where are you? please follow me."),
                    transitions={
                        "succeeded": f"CHECK_AROUND_{i}_{state-2}",
                        "aborted": "failed",
                        "preempted": "failed",
                    },
                )
            ...
            # 3. Final destination
            end_pose = Pose(
                position=Point(**locations[-1]["pose"]["position"]),
                orientation=Quaternion(**locations[-1]["pose"]["orientation"]),
            )
            smach.StateMachine.add(
                f"GUIDING_DESTINATION",
                GoToLocation(end_pose),
                transitions={
                    "succeeded": "TURN_BACK",
                    "aborted": "failed",
                    "preempted": "failed",
            ...
            end_turn_pose = Pose(
                position=Point(**turning_point[-1]["pose"]["position"]),
                orientation=Quaternion(**turning_point[-1]["pose"]["orientation"]),
            )
            smach.StateMachine.add(
                f"TURN_BACK",
                GoToLocation(end_turn_pose),
                transitions={
            ...
            smach.StateMachine.add(
                "SAY_ARRIVED",
                Say(text="We've arrived at the destination. See you."),
                transitions={
                    "succeeded": "succeeded",
                    "aborted": "failed",
                    "preempted": "failed",
...
if __name__ == "__main__":
    ...
    location = [
        rospy.get_param("/Start"),
        rospy.get_param("/WaitPoint1"),
        rospy.get_param("/WaitPoint2"),
        rospy.get_param(f"/Destination"),
    ]
    turn_around = [
        rospy.get_param(f"/TurnAround1"),
        rospy.get_param(f"/TurnAround2"),
        rospy.get_param("/TurnAroundDestination"),
    ]
