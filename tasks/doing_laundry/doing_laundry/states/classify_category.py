...
# Ideally these live in your config yaml under doing_laundry.objects.<name>.category
CATEGORY_MAP = {
    "fruit": {
        "apple",
        "banana",
...
class ClassifyCategory(yasmin.State):
    ...
    Classification priority:
        1. ROS 2 param lookup  (doing_laundry.objects.<name>.category)
    ...
    def _get_category(self, name: str) -> str | None:
        ...
        try:
            category = (
                self.node.get_parameter(f"doing_laundry.objects.{name}.category")
                .get_parameter_value()
                .string_value
            )
            if category:
