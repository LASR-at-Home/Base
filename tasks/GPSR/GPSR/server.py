...
TARGET_API_URL = "http://127.0.0.1:1337/v1/chat/completions"  # Example: Ollama local server
# Example alternatives: 
# "https://api.deepseek.com/v1/chat/completions"
# "https://openrouter.ai/api/v1/chat/completions"

# Fallback API key if the incoming request doesn't provide one
DEFAULT_TARGET_API_KEY = "sk-TYUIOKJHBVBN"
...
async def proxy_chat_completions(request: Request):
    ...
    if body.get("stream", False):
        async def stream_generator():
            try:
                async with client.stream("POST", TARGET_API_URL, json=body, headers=forward_headers) as response:
                    if response.status_code != 200:
                        yield f"Error from upstream: {response.status_code}".encode('utf-8')
                        return
    ...
    # 5. Handle Standard Responses (stream: false)
        ...
        try:
            async with client as c:
                response = await c.post(TARGET_API_URL, json=body, headers=forward_headers)
            ...
            raise HTTPException(status_code=500, detail=f"An error occurred while requesting upstream: {exc}")
