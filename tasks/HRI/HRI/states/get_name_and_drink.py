...
class GetNameAndDrink(yasmin.StateMachine):
    class ParseNameAndDrink(yasmin_ros.ServiceState):
        ...
        def _handle_resp(self, blackboard, result):
            blackboard['guest_data'].update({self.guest_id: {"name": result.name}}) if self.task == "name" else blackboard['guest_data'].update({self.guest_id: {"drink": result.favoutrite_drink}})
    ...
    class PostRecoveryDecision(yasmin.State):
        ...
        def _recovery_name_and_drink_required(self, blackboard) -> bool:
            ...
            return blackboard['guest_data'][self._guest_id]["name"] == "unknown" and blackboard['guest_data'][self._guest_id]["drink"] == "unknown"
    ...
    def __init__(
        ...
        super().__init__(
            outcomes=["succeeded", "failed", "failed_name", "failed_drink"], handle_sigint=True
        )
