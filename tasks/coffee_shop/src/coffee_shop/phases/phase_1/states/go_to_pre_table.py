...
class GoToPreTable(smach.State):
    ...
    def execute(self, userdata):
        ...
        closest_table = min(unvisited, key=lambda table: np.linalg.norm([table[1]["pre_location"]["position"]["x"] - robot_x, table[1]["pre_location"]["position"]["y"] - robot_y]))
        label, next_table = closest_table
        self.context.voice_controller.async_tts(f"I am going to {label}")
        position, orientation = next_table["pre_location"]["position"], next_table["pre_location"]["orientation"]
