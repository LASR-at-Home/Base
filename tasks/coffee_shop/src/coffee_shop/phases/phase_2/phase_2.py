...
class Phase2(smach.StateMachine):
    def __init__(self, base_controller, head_controller, voice_controller):
        ...
        with self:
            ...
            smach.StateMachine.add('MAKE_ORDER', MakeOrder(voice_controller), transitions={'done' : 'DELIVER_ORDER'})
            smach.StateMachine.add('CHECK_ORDER', CheckOrder(voice_controller), transitions={'done' : 'DELIVER_ORDER'})
